...
def main():
    ...
    print("\n加载数据...")
    try:
        df = load_data('lottery_history_recalculated.csv')
        print(f"✓ 加载了 {len(df)} 条记录")
    except FileNotFoundError:
        print("✗ 错误：找不到数据文件 lottery_history_recalculated.csv")
        return
    ...
    # 保存模型
    model_file = f"zodiac_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
